...
export const useAuth = create((set) => ({
  ...
  logout: async () => {
    try {
      //set loading state
      //make logout api req
      let res = await axios.get("https://blogapp-4hbr.onrender.com/auth/logout", { withCredentials: true });
      //update state
      if (res.status === 200) {
        set({
          currentUser: null,
  ...
  checkAuth: async () => {
    try {
      set({ loading: true });
      const res = await axios.get("https://blogapp-4hbr.onrender.com/auth/check-auth", { withCredentials: true });
